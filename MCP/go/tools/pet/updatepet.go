...
func UpdatepetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/pet", cfg.BaseURL)
...
func CreateUpdatepetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_pet",
		mcp.WithDescription("Update an existing pet"),
		mcp.WithArray("photoUrls", mcp.Required(), mcp.Description("")),
		mcp.WithString("status", mcp.Description("Input parameter: pet status in the store")),
		mcp.WithArray("tags", mcp.Description("")),
		mcp.WithObject("category", mcp.Description("")),
