...
func UpdateuserHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdateuserTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_user_username",
		mcp.WithDescription("Updated user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("name that need to be deleted")),
		mcp.WithString("password", mcp.Description("")),
		mcp.WithString("phone", mcp.Description("")),
		mcp.WithNumber("userStatus", mcp.Description("Input parameter: User Status")),
